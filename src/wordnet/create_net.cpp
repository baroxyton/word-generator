...
network::Network network::create_net(std::string filename,
  ...
  Network output;
  output.layers.push_back(new std::vector<weighted_token*>{
      new weighted_token{new tokenizers::token{0, "<BEGIN>"}, new std::vector<double>(0)}});
  ...
  for (auto word : wordlist) {
    ...
    for (auto word_token : word_tokens) {
      ...
      if(find_layer_tokindex(output.layers[i+1], word_token.id) == -1){
        try{
        output.layers[i+1]->push_back(new weighted_token{new tokenizers::token{word_token.id, word_token.token}, new std::vector<double>(output.layers[i+2]->size())});
        }
        catch(std::bad_alloc){
     ...
     output.layers[i]->at(find_layer_tokindex(output.layers[i], last_token))->weigths_out->at(find_layer_tokindex(output.layers[i+1], word_token.id))++;
      i++;
    }
  }
